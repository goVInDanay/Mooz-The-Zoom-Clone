...
// Define a type for the meeting data
type Meeting = {
  id: string;
  startsAt: Date;
  link: string;
...
const MeetingTypes = ({ onLatestMeetingChange }: { onLatestMeetingChange: (meeting: Meeting | null) => void }) => {
  ...
  const createMeeting = async () => {
    ...
    try {
      ...
      const newMeeting: Meeting = {
        ...
        link: `${window.location.protocol}//${window.location.host}/meeting/${call.id}`,
  ...
  // Determine the latest upcoming meeting
  useEffect(() => {
    if (meetings.length > 0) {
      const sortedMeetings = [...meetings].sort((a, b) => a.startsAt.getTime() - b.startsAt.getTime());
      const latestMeeting = sortedMeetings[0];
      onLatestMeetingChange(latestMeeting);
    } else {
      onLatestMeetingChange(null);
    }
  }, [meetings, onLatestMeetingChange]);
  ...
  return (
    <section className='grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4'>
      ...
      <HomeCard
        ...
        description='Watch Your Recordings'
      ...
      <HomeCard
        ...
        title='Join Meeting'
        description='Join Meeting Using Link'
